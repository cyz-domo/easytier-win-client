...
impl RuntimeManager {
    ...
    pub fn snapshot(&mut self, cfg: &InstanceConfig) -> InstanceSnapshot {
        ...
        InstanceSnapshot { id: cfg.id.clone(), name: cfg.name.clone(), rpc_port: cfg.rpc_port, desired_state: cfg.desired_state, observed_state: if running { ObservedState::Running } else if error.is_some() { ObservedState::Failed } else { ObservedState::Stopped }, pid: self.children.get(&cfg.id).map(Child::id), last_error: error }
    }
    pub fn start(&mut self, cfg: &InstanceConfig) -> Result<InstanceSnapshot, String> {
        ...
        self.errors.remove(&cfg.id); self.children.insert(cfg.id.clone(), child);
        for _ in 0..20 {
            if std::net::TcpStream::connect(("127.0.0.1", cfg.rpc_port)).is_ok() { return Ok(self.snapshot(cfg)); }
