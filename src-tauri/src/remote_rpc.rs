...
/// recovery, service).
pub fn build_rpc_portal_args(remote_manage_enabled: bool, rpc_port: u16, whitelist_cidrs: &[String]) -> Vec<String> {
...
mod tests {
    ...
    fn portal_args_remote_mode_dedups_and_skips_blank() {
        let args = build_rpc_portal_args(true, 15888, &["  ".to_string(), "127.0.0.1/32".to_string(), "10.0.0.0/8".to_string()]);
        let list = args[1].trim_start_matches("--rpc-portal-whitelist=");
