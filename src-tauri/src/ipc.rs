...
pub fn response(req: &Request, data: Value) -> Response {
    Response {
...
pub fn error(req: &Request, code: &str, message: impl Into<String>) -> Response {
    Response {
        protocol_version: PROTOCOL_VERSION,
        request_id: req.request_id.clone(),
        ok: false,
...
pub fn read_request<R: BufRead>(reader: &mut R) -> Result<Request, String> {
    ...
    let request: Request =
        serde_json::from_slice(&line).map_err(|e| format!("invalid_request: {e}"))?;
    if request.protocol_version != PROTOCOL_VERSION
        || request.request_id.trim().is_empty()
        || request.command.trim().is_empty()
    {
        return Err("invalid_request: invalid protocol_version, request_id, or command".into());
    }
    Ok(request)
}
pub fn write_response<W: Write>(writer: &mut W, response: &Response) -> Result<(), String> {
...
mod windows_security {
    ...
    use windows_sys::Win32::{
        Foundation::{CloseHandle, GetLastError, LocalFree, HANDLE, INVALID_HANDLE_VALUE},
    ...
    fn winerr(ctx: &str) -> String {
        format!("{ctx}: Windows error {}", unsafe { GetLastError() })
    }
    ...
    fn sid_string(sid: *mut core::ffi::c_void) -> Result<String, String> {
        ...
        unsafe {
            LocalFree(text as *mut core::ffi::c_void);
        }
        Ok(value)
    }
    pub fn validate_sid(sid: &str) -> Result<(), String> {
        let mut parsed = core::ptr::null_mut();
        if unsafe { ConvertStringSidToSidW(wide(sid).as_ptr(), &mut parsed) } == 0 {
            return Err(winerr("invalid interactive user SID"));
        }
        unsafe {
            LocalFree(parsed as *mut core::ffi::c_void);
        }
        Ok(())
    }
    pub fn create_pipe(sid: &str) -> Result<File, String> {
        ...
        {
            return Err(winerr("create named pipe security descriptor"));
        }
        ...
        if handle == INVALID_HANDLE_VALUE {
            return Err(winerr("create named pipe"));
        }
        Ok(unsafe { File::from_raw_handle(handle as _) })
    }
    pub fn connect(pipe: &File) -> Result<(), String> {
        if unsafe { ConnectNamedPipe(pipe.as_raw_handle() as HANDLE, core::ptr::null_mut()) } == 0
            && unsafe { GetLastError() } != 535
        {
            return Err(winerr("wait for named pipe client"));
        }
        Ok(())
    }
    pub fn allowed(pipe: &File, configured_sid: &str) -> Result<bool, String> {
        ...
        if unsafe { GetNamedPipeClientProcessId(pipe.as_raw_handle() as HANDLE, &mut pid) } == 0 {
            return Err(winerr("get client PID"));
        }
        let process = unsafe { OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, 0, pid) };
        if process == 0 {
            return Err(winerr("open client process"));
        }
        let mut token = 0;
        ...
        if ok == 0 {
            return Err(winerr("open client token"));
        }
        let result = token_allowed(token, configured_sid);
        unsafe {
            CloseHandle(token);
        }
        result
    ...
    fn token_allowed(token: HANDLE, configured_sid: &str) -> Result<bool, String> {
        ...
        if needed == 0 {
            return Err(winerr("query token user size"));
        }
        let mut buf = vec![0u8; needed as usize];
        if unsafe {
            GetTokenInformation(token, TokenUser, buf.as_mut_ptr() as _, needed, &mut needed)
        } == 0
        {
            return Err(winerr("query token user"));
        }
        let user_sid = unsafe { (*(buf.as_ptr() as *const TOKEN_USER)).User.Sid };
        let user_text = sid_string(user_sid)?;
        if user_text == configured_sid || user_text == "S-1-5-32-544" || user_text == "S-1-5-18" {
            return Ok(true);
        }
        let mut expected = core::ptr::null_mut();
        if unsafe { ConvertStringSidToSidW(wide(configured_sid).as_ptr(), &mut expected) } == 0 {
            return Err(winerr("parse configured SID"));
        }
        let match_user = unsafe { windows_sys::Win32::Security::EqualSid(user_sid, expected) != 0 };
        unsafe {
            LocalFree(expected);
        }
        if match_user {
        ...
        let mut needed = 0;
        unsafe {
            GetTokenInformation(token, TokenGroups, core::ptr::null_mut(), 0, &mut needed);
        }
        if needed == 0 {
            return Ok(false);
        }
        let mut groups_buf = vec![0u8; needed as usize];
        if unsafe {
            GetTokenInformation(
                token,
                TokenGroups,
                groups_buf.as_mut_ptr() as _,
                needed,
                &mut needed,
            )
        } == 0
        {
            return Err(winerr("query token groups"));
        }
        ...
        for i in 0..groups.GroupCount as usize {
            ...
            if unsafe { ConvertSidToStringSidW(group.Sid, &mut text) } == 0 {
                return Err(winerr("convert group SID"));
            }
            let mut n = 0;
            unsafe {
                while *text.add(n) != 0 {
            ...
            if value == configured_sid || value == "S-1-5-32-544" || value == "S-1-5-18" {
                return Ok(true);
            }
        }
        Ok(false)
...
where
    F: FnMut(Request) -> Response,
{
    Err("Named Pipe IPC is only available on Windows".into())
}
